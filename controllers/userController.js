...
const loginUser = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ email, token, role: user.role, firstName: user.firstName, lastName: user.lastName, phone: user.phone, userId: user._id })
    } catch (error) {
        res.status(400).json({ error: error.message })
    }
...
const signupUser = async (req, res) => {
    ...
    try {
        ...
        res.status(200).json({ email, token, role: 401, userId: user._id });
    } catch (error) {
